...
class Population_Noah : public Population<T, L>{

private:

    // Function taking a vector of genes of type T, a child T and a diversity preserver and a diversity preserver
    std::function<Diversity_Preserver<T>(const std::vector<T>&, const std::vector<L>&, const std::vector<T>&, const Diversity_Preserver<T>&, std::mt19937&)>& selectSurvivors_Div;
...
public:
    ...
    void set_selectSurvivors_Div(const std::function<Diversity_Preserver<T>(const std::vector<T>&, const std::vector<L>&, const std::vector<T>&, const Diversity_Preserver<T>&, std::mt19937&)>& selectSurvivors_Div);
};

// Class Implementation ---------------------------------------------------------------------------------------------------------------------

template <typename T, typename L>
Population_Noah<T, L>::Population_Noah(
    int seed,
    std::function<std::vector<T>(std::mt19937&)>& initialize,
    std::function<std::vector<L>(const std::vector<T>&)>& evaluate,
    std::function<std::vector<T>(const std::vector<T>&, const std::vector<L>&, std::mt19937&)>& selectParents,
...
template <typename T, typename L>
void Population_Noah<T, L>::execute() {
    this->generation++;
    ...
    if(selectSurvivors_Div != nullptr){
        div_preserver = selectSurvivors_Div(this->genes, fitnesses, children, div_preserver, this->generator);
        this->genes = div_preserver.genes;
    }
}

template <typename T, typename L>
void Population_Noah<T, L>::set_selectSurvivors_Div(const std::function<Diversity_Preserver<T>(const std::vector<T>&, const std::vector<L>&, const std::vector<T>&, const Diversity_Preserver<T>&, std::mt19937&)>& selectSurvivors_Div){ this->selectSurvivors_Div = selectSurvivors_Div;}